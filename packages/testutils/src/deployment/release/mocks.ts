...
export interface MockDeploymentOutput {
  ...
  uniswapV2Derivatives: Promise<{
    risqWeth: MockUniswapV2PriceSource;
...
export const deployMocks = describeDeployment<MockDeploymentConfig, MockDeploymentOutput>({
  ...
  async tokens(config) {
    ...
    const [bat, bnb, bnt, comp, dai, knc, link, mana, risq, ren, rep, uni, usdc, usdt, zrx, mrt] = await Promise.all([
      ...
      MockToken.deploy(config.deployer, 'Risq Token', 'RISQ', 18),
  ...
  async uniswapV2Derivatives(config, deployment) {
    const tokens = await deployment.tokens;
    const [risqWeth, kncWeth, usdcWeth] = await Promise.all([
    ...
    return { risqWeth, kncWeth, usdcWeth };
  },
  // Price feed sources
  ...
  async uniswapV2Integratee(config, deployment) {
    ...
    return MockUniswapV2Integratee.deploy(
      config.deployer,
      [tokens.risq, tokens.knc],
      [tokens.weth, tokens.weth],
      [derivatives.risqWeth, derivatives.kncWeth],
      await deployment.centralizedRateProvider,
      0,
    );
  },
...
export async function configureMockRelease({
  ...
  const uniswapV2Derivatives = [mocks.uniswapV2Derivatives.risqWeth, mocks.uniswapV2Derivatives.kncWeth];
  ...
  const chainlinkAggregators = [
    ...
    mocks.chainlinkAggregators.risq,
  ...
  return {
    ...
    derivatives: {
      ...
      uniswapV2: {
        risqWeth: mocks.uniswapV2Derivatives.risqWeth,
