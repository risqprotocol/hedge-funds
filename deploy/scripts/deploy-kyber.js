...
const main = async input => {
  ...
  const risq = fetchContract('StandardToken', input.tokens.addr.RISQ);
  ...
  const risqData = await call(conversionRates, 'getTokenBasicData', [risq.options.address]);
  if (!risqData || !risqData['0']) {
    await send(conversionRates, 'addToken', [risq.options.address]);
  }
  await send(conversionRates, 'setTokenControlInfo', [risq.options.address, minimalRecordResolution, maxPerBlockImbalance.toString(), maxTotalImbalance.toString()]);
  await send(conversionRates, 'enableTokenTrade', [risq.options.address]);
  ...
  await send(kyberReserve, 'approveWithdrawAddress', [risq.options.address, conf.deployer, true]);
  await send(kyberReserve, 'enableTrade');

  const kyberReserveRisqBalance = await call(risq, 'balanceOf', [kyberReserve.options.address]);
  if (`${kyberReserveRisqBalance}` === '0') {
    await send(risq, 'transfer', [kyberReserve.options.address, tokensToTransfer.toString()]);
  }
  ...
  await send(conversionRates, 'setBaseRate', [[risq.options.address], [tokensPerEther.toString()], [ethersPerToken.toString()], ['0x0000000000000000000000000000'], ['0x0000000000000000000000000000'], blockNumber, [0]]);
  await send(conversionRates, 'setQtyStepFunction', [risq.options.address, [0], [0], [0], [0]]);
  await send(conversionRates, 'setImbalanceStepFunction', [risq.options.address, [0], [0], [0], [0]]);
  ...
  await send(kyberNetwork, 'listPairForReserve', [kyberReserve.options.address, risq.options.address, true, true, true]);
