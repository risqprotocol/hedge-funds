...
let deployer;
let defaultTxOpts;
let testingPriceFeed, weth, risq;
let fund;
...
beforeAll(async () => {
  ...
  await send(
    testingPriceFeed,
    'update',
    [[weth.options.address, risq.options.address], [exaUnit, exaUnit]],
    defaultTxOpts
  );
...
describe('updateOwnedAssets', () => {
  test('removes zero balance asset', async () => {
    ...
    const risqInvestAmt = '10000000';
    const wantedShares = risqInvestAmt;
    const pbguAmount = toWei('0.01', 'ether');

    await send(
      risq,
      'approve',
      [participation.options.address, risqInvestAmt],
      defaultTxOpts
    );
    await send(
      participation,
      'requestInvestment',
      [wantedShares, risqInvestAmt, risq.options.address],
