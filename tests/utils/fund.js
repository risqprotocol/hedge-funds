...
export const getFundComponents = async backOfficeAddress => {
  const components = {};
  components.backOffice = fetchContract('BackOffice', backOfficeAddress);
  const routes = await call(components.backOffice, 'routes');
...
export const setupFundWithParams = async ({
  ...
  // TODO: need to calculate pbgu estimates here instead of passing in arbitrary value
  if (!pbguTxValue) {
    pbguTxValue = toWei('0.01', 'ether')
  }
  const managerTxOptsWithPbgu = { ...managerTxOpts, value: pbguTxValue };
  await send(
    version,
    'beginSetup',
    [
  ...
  await send(version, 'createAccounting', [], managerTxOptsWithPbgu);
  await send(version, 'createFeeManager', [], managerTxOptsWithPbgu);
  await send(version, 'createParticipation', [], managerTxOptsWithPbgu);
  await send(version, 'createPolicyManager', [], managerTxOptsWithPbgu);
  await send(version, 'createShares', [], managerTxOptsWithPbgu);
  await send(version, 'createTrading', [], managerTxOptsWithPbgu);
  await send(version, 'createVault', [], managerTxOptsWithPbgu);
  const res = await send(version, 'completeSetup', [], managerTxOptsWithPbgu);
  ...
  if (new BN(initialInvestment.contribAmount).gt(new BN(0))) {
    ...
    // const pbguAmount = toWei('.1', 'ether');
    ...
    await send(
      ...
      { ...investorTxOpts, value: pbguTxValue }
    )
    await send(
      fund.participation,
      'executeRequestFor',
...
export const setupInvestedTestFund = async (contracts, manager, pbguTxValue = null) => {
  const [deployer] = await getAccounts();
  const managerTxOpts = { from: manager, gas: 8000000 };
