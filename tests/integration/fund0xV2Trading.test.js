...
let contracts;
let risq, zrx, weth, erc20Proxy, zeroExExchange;
let fund;
...
describe('Fund takes an order', () => {
  ...
  test('third party makes and validates an off-chain order', async () => {
    ...
    const unsignedOrder = await createUnsignedZeroExOrder(
      ...
      {
        makerAddress,
        makerTokenAddress: risq.options.address,
        makerAssetAmount,
...
describe('Fund cancels an order', () => {
  ...
  test('Fund can cancel the order', async () => {
    ...
    const makerAssetAllowance = new BN(
      await call(risq, 'allowance', [trading.options.address, erc20Proxy.options.address])
    );
