...
let contracts;
let fund, weth, risq, priceSource, maliciousToken;
...
beforeAll(async () => {
  ...
  fund = await setupFundWithParams({
    defaultTokens: [risq.options.address, weth.options.address, maliciousToken.options.address],
    initialInvestment: {
...
test('Redeem with constraints works as expected', async () => {
  ...
  expect(postRisqVault).toEqual(preRisqVault);
  expect(postRisqInvestor).toEqual(preRisqInvestor);
