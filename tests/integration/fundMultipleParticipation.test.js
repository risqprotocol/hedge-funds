...
describe('Fund 1: Multiple investors buying shares with different tokens', () => {
  let pbguAmount, shareSlippageTolerance;
  let wantedShares1, wantedShares2, wantedShares3;
  let daiToEthRate, risqToEthRate, wethToEthRate;
  let dai, risq, priceSource, weth;
  let fund;
  ...
  beforeAll(async () => {
    ...
    risqToEthRate = toWei('0.5', 'ether');
    daiToEthRate = toWei('0.005', 'ether');
    await send(
      priceSource,
      'update',
      [
        [weth.options.address, risq.options.address, dai.options.address],
        [wethToEthRate, risqToEthRate, daiToEthRate],
      ],
      defaultTxOpts
    );
  ...
  test('A user can have only one pending investment request', async () => {
    ...
    await send(
      ...
      { ...investor1TxOpts, value: pbguAmount }
    );
  ...
  test('Investment request allowed for second user with another default token', async () => {
    ...
    const offerAsset = risq.options.address;
    const expectedOfferAssetCost = new BN(
      await call(
        accounting,
        'getShareCostInAsset',
    ...
    // Investor 2 - risq
    await send(risq, 'transfer', [investor2, offerAssetMaxQuantity], defaultTxOpts);
    await send(
      risq,
      'approve',
      [participation.options.address, offerAssetMaxQuantity],
    ...
    await send(
      ...
      { ...investor2TxOpts, value: pbguAmount }
    );
  });
  ...
  test('Investment request allowed for third user with approved token', async () => {
    ...
    await expect(
      send(
        ...
        { ...investor3TxOpts, value: pbguAmount }
      )
