...
let dai, knc, risq, weth, zrx, oasisDex, oasisDexAdapter, priceSource, priceTolerance;
let contracts;
...
beforeAll(async () => {
  ...
  const wethToRisqRate = toWei('0.5', 'ether');
...
describe('Fund 1: Asset blacklist, price tolerance, max positions, max concentration', () => {
  ...
  describe('Asset blacklist', () => {
    ...
    test('Good maker order: non-blacklisted taker asset', async () => {
      ...
      await send(risq, 'transfer', [investor, takerQuantity], defaultTxOpts);
      await send(
        risq,
        'approve',
  ...
  describe('Price tolerance', () => {
    ...
    test('Good make order: slippage just within limit', async () => {
      ...
      const orderId = hexToNumber(logMake.id);
      await send(risq, 'transfer', [investor, toleratedTakerQuantity], defaultTxOpts);
      await send(
        risq,
        'approve',
...
describe('Fund 2: Asset whitelist, max positions', () => {
  ...
  beforeAll(async () => {
    ...
    assetWhitelist = await deploy(
      ...
      [[dai.options.address, risq.options.address, zrx.options.address]]
    );
    const currentPositions = await call(accounting, 'getOwnedAssetsLength');
  ...
  describe('Max positions', () => {
    test('Good take order: final allowed position', async () => {
      ...
      await send(risq, 'transfer', [investor, makerQuantity], defaultTxOpts);
      await send(risq, 'approve', [oasisDex.options.address, makerQuantity], investorTxOpts);
      const receipt = await send(
        oasisDex,
        'offer',
