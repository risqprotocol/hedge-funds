...
let contracts;
let dai, risq, weth, zrx, priceSource, version, zeroExExchange, erc20Proxy, fund, zeroExAdapter;
let exchangeIndex;
...
describe('Fund takes an order', () => {
  ...
  test('Third party makes an order', async () => {
    ...
    const makerTokenAddress = risq.options.address;
    ...
    await send(risq, 'approve', [erc20Proxy.options.address, makerAssetAmount], defaultTxOpts);
  ...
  test('Manager takes order through 0x adapter', async () => {
    ...
    const preRisqFund = new BN(await call(risq, 'balanceOf', [fund.vault.options.address]));
    ...
    const postRisqFund = new BN(await call(risq, 'balanceOf', [fund.vault.options.address]));
    ...
    expect(postRisqDeployer).bigNumberEq(preRisqDeployer.sub(new BN(signedOrder.makerAssetAmount)));
    expect(postWethVault).bigNumberEq(
      preWethVault
        .sub(new BN(signedOrder.takerAssetAmount))
        .sub(new BN(protocolFeeAmount))
    );
    expect(postRisqFund).bigNumberEq(preRisqFund.add(new BN(signedOrder.makerAssetAmount)));
...
describe('Fund takes an order with a taker fee', () => {
  ...
  test('Fund with enough taker fee asset takes order', async () => {
    ...
    expect(postRisqVault).bigNumberEq(preRisqVault.add(new BN(signedOrder.makerAssetAmount)));
    expect(postWethDeployer).bigNumberEq(preWethDeployer.add(new BN(signedOrder.takerAssetAmount)));
...
describe('Fund makes an order (no maker/taker fees)', () => {
  ...
  test('Fund makes an order', async () => {
    ...
    const takerTokenAddress = risq.options.address;
  ...
  test('Third party takes the order made and accounting updated', async () => {
    ...
    await send(risq, 'approve', [erc20Proxy.options.address, signedOrder.takerAssetAmount], defaultTxOpts);
    ...
    expect(postRisqFundHoldings).bigNumberEq(
      preRisqFundHoldings.add(new BN(signedOrder.takerAssetAmount))
    );
    expect(postZrxFundHoldings).bigNumberEq(
      preZrxFundHoldings.sub(new BN(signedOrder.makerAssetAmount))
    );
    expect(postRisqDeployer).bigNumberEq(
      preRisqDeployer.sub(new BN(signedOrder.takerAssetAmount))
    );
...
describe('Fund makes an order with a maker and taker fee', () => {
  ...
  test('Fund makes an order', async () => {
    ...
    const makerAssetAllowance = new BN(
      await call(risq, 'allowance', [makerAddress, erc20Proxy.options.address])
    )
  ...
  test('Third party takes the order and accounting updated', async () => {
    ...
    // Update accounting so maker asset is no longer marked as in an open order
    await send(trading, 'updateAndGetQuantityBeingTraded', [risq.options.address], managerTxOpts);

    const isInOpenMakeOrder = await call(trading, 'isInOpenMakeOrder', [risq.options.address]);
    expect(isInOpenMakeOrder).toEqual(false);

    expect(postRisqFundHoldings).bigNumberEq(
      preRisqFundHoldings.sub(new BN(signedOrder.makerAssetAmount))
    );
    expect(postWethFundHoldings).bigNumberEq(
      preWethFundHoldings.add(new BN(signedOrder.takerAssetAmount))
    );
    expect(postRisqDeployer).bigNumberEq(
      preRisqDeployer.add(new BN(signedOrder.makerAssetAmount))
    );
...
describe('Fund can take an order when protocol fee disabled', () => {
  ...
  test('Manager takes order through 0x adapter', async () => {
    ...
    const preRisqDeployer = new BN(await call(risq, 'balanceOf', [deployer]));
    const preRisqFund = new BN(await call(accounting, 'assetHoldings', [risq.options.address]));
    const preWethDeployer = new BN(await call(weth, 'balanceOf', [deployer]));
    ...
    const postRisqDeployer = new BN(await call(risq, 'balanceOf', [deployer]));
    const postRisqFund = new BN(await call(accounting, 'assetHoldings', [risq.options.address]));
    const postWethDeployer = new BN(await call(weth, 'balanceOf', [deployer]));
