/*
 * @file Tests a fund trading with the Risq Engine
 *
...
let engine, risq, fund, weth, engineAdapter, priceSource, priceTolerance;
let contracts;
let exchangeIndex, risqPrice, takerQuantity;
...
beforeAll(async () => {
  ...
  risqPrice = (await priceSource.methods
    .getPrice(risq.options.address)
    .call())[0];
  takerQuantity = toWei('0.001', 'ether'); // Risq sell qty
});

test('Setup a fund with pbgu charged to seed Risq Engine', async () => {
  await engine.methods.setPbguPrice(toWei('1', 'gwei')).send(defaultTxOpts);

  // TODO: Need to calculate this in fund.js
  const pbguTxValue = toWei('10', 'ether');
  fund = await setupInvestedTestFund(contracts, user, pbguTxValue);
...
test('Trade on Risq Engine', async () => {
  ...
  const preFundWeth = await weth.methods
    ...
    .call();
  const preFundRisq = await risq.methods
    .balanceOf(fund.vault.options.address)
    .call();
  ...
  const postFundWeth = await weth.methods
    .balanceOf(fund.vault.options.address)
    .call();
  const postFundRisq = await risq.methods
    .balanceOf(fund.vault.options.address)
    .call();

  expect(
    new BN(preFundRisq.toString())
      .sub(new BN(postFundRisq.toString()))
...
test('Maker quantity as minimum returned WETH is respected', async () => {
  const makerQuantity = new BN(risqPrice.toString()).div(new BN(2)).toString();
