...
describe('Happy Path', () => {
  ...
  beforeAll(async () => {
    ...
    const { 0: risqPrice } = await testingPriceFeed.methods
      .getPrice(risq.options.address)
      .call();
    const ethPriceInRisq = BNExpInverse(new BN(risqPrice.toString())).toString()
  ...
  test('Trade on kyber', async () => {
    ...
    const { 1: expectedRate } = await kyberNetworkProxy.methods
      .getExpectedRate(KYBER_ETH_ADDRESS, risq.options.address, takerQuantity)
      .call(defaultTxOpts);
    ...
    const preRisqBalance = await risq.methods
    ...
    const postRisqBalance = await risq.methods
      .balanceOf(fund.vault.options.address)
      .call();

    const risqBalanceDiff = new BN(postRisqBalance.toString()).sub(new BN(preRisqBalance.toString()));
    expect(risqBalanceDiff.gt(new BN(makerQuantity))).toBe(true);
