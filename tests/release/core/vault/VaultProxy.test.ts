...
import { IMigrationHookHandler, VaultLib } from '@risqprotocol/protocol';
import { addNewAssetsToFund, assertEvent, createNewFund, defaultTestDeployment } from '@risqprotocol/testutils';
...
describe('removeTrackedAsset', () => {
  ...
  it('works as expected', async () => {
    ...
    await weth.transfer(vaultProxy, utils.parseEther('1'));
    await risq.transfer(vaultProxy, utils.parseEther('1'));
    await knc.transfer(vaultProxy, utils.parseEther('1'));
    await mockVaultAccessor.forward(vaultProxy.addTrackedAsset, weth);
    await mockVaultAccessor.forward(vaultProxy.addTrackedAsset, risq);
    await mockVaultAccessor.forward(vaultProxy.addTrackedAsset, knc);

    const receipt1 = await mockVaultAccessor.forward(vaultProxy.removeTrackedAsset, risq);
