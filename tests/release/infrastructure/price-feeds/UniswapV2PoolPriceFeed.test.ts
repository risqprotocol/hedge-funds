...
import { IUniswapV2Pair, MockToken, MockUniswapV2PriceSource, StandardToken } from '@risqprotocol/protocol';
...
async function snapshot(provider: EthereumTestnetProvider) {
  ...
  const risq = deployment.tokens.risq;
  ...
  // Create a pair formed by same decimal tokens (risq/weth)
  const risqWethPair = await MockUniswapV2PriceSource.deploy(
    config.deployer,
    deployment.centralizedRateProvider,
    risq,
    weth,
  );
  await deployment.uniswapV2PoolPriceFeed.addPoolTokens([risqWethPair]);
  await deployment.aggregatedDerivativePriceFeed.addDerivatives([risqWethPair], [deployment.uniswapV2PoolPriceFeed]);

  await risq.transfer(risqWethPair, utils.parseEther(defaultSeedAmount));
  await weth.transfer(risqWethPair, utils.parseEther(defaultSeedAmount));

  return {
    accounts,
    deployment,
    mocks: { risqWethPair, wethUsdcPair },
    defaultSeedAmount,
    config,
  };
}
...
describe('calcUnderlyingValues', () => {
  it('returns rate for 18 decimals underlying assets', async () => {
    ...
    const uniswapWethUsdtPairToken = new StandardToken(risqWethPair, provider);
    ...
    const calcUnderlyingValues = await uniswapV2PoolPriceFeed.calcUnderlyingValues
      .args(risqWethPair, utils.parseUnits('1', await risqWethPair.decimals()))
      .call();
    const ratePrecision = BigNumber.from(10).pow(18);

    const wethAmount = utils.parseEther(defaultSeedAmount);
    const risqAmount = utils.parseEther(defaultSeedAmount);
    ...
    expect(calcUnderlyingValues).toMatchFunctionOutput(uniswapV2PoolPriceFeed.calcUnderlyingValues, {
      underlyingAmounts_: [
        ...
        risqAmount.mul(ratePrecision).div(totalSupply),
      ],
      underlyings_: [risq, weth],
    });
  });
  ...
  it('adds pool tokens and emits an event per added pool token', async () => {
    ...
    expect(await uniswapV2PoolPriceFeed.getPoolTokenInfo(primitive0Primitive1Pair)).toMatchFunctionOutput(
      uniswapV2PoolPriceFeed.getPoolTokenInfo,
      {
        ...
        token1Decimals: await risq.decimals(),
      },
    );
