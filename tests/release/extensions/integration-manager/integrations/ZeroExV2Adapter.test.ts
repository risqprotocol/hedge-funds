...
import { defaultTestDeployment, createNewFund } from '@risqprotocol/testutils';
...
describe('parseAssetsForMethod', () => {
  it('does not allow a maker which is not whitelisted', async () => {
    const {
      ...
      deployment: {
        tokens: { risq: incomingAsset, weth: outgoingAsset },
        zeroExV2Adapter,
      },
    } = await provider.snapshot(snapshot);
  ...
  it('generates expected output with takerFeeAsset that is the same as makerAsset', async () => {
    const {
      ...
      deployment: {
        tokens: { zrx: incomingAsset, risq: outgoingAsset },
        zeroExV2Adapter,
      },
    } = await provider.snapshot(snapshot);
  ...
  it('generates expected output with takerFeeAsset that is the same as takerAsset', async () => {
    const {
      ...
      deployment: {
        tokens: { risq: incomingAsset, zrx: outgoingAsset },
        zeroExV2Adapter,
      },
    } = await provider.snapshot(snapshot);
  ...
  it('generates expected output with takerFee', async () => {
    const {
      ...
      deployment: {
        tokens: { risq: incomingAsset, weth: outgoingAsset, zrx: takerFeeAsset },
        zeroExV2Adapter,
      },
    } = await provider.snapshot(snapshot);
