...
async function snapshot(provider: EthereumTestnetProvider) {
  ...
  const token1 = deployment.tokens.risq;
  const token0 = deployment.tokens.weth;

  const mockPair = config.derivatives.uniswapV2.risqWeth;
...
describe('parseAssetsForMethod', () => {
  it('does not allow a bad selector', async () => {
    const {
      config: { risq: tokenA, weth: tokenB },
  ...
  it('works as expected when called by a fund', async () => {
    const {
      ...
      deployment: {
        uniswapV2Adapter,
        integrationManager,
        tokens: { risq: tokenA, weth: tokenB },
      },
  ...
  it('works as expected when called by a fund', async () => {
    const {
      deployment: {
        uniswapV2Adapter,
        integrationManager,
        tokens: { risq: outgoingAsset, dai: incomingAsset },
      },
      fund: { comptrollerProxy, fundOwner, vaultProxy },
    } = await provider.snapshot(snapshot);
