...
import { addNewAssetsToFund, defaultTestDeployment, assertEvent, createNewFund } from '@risqprotocol/testutils';
  ...
  describe('__addZeroBalanceTrackedAssets', () => {
    it('only allows authorized users', async () => {
      const {
        ...
        deployment: {
          integrationManager,
          tokens: { risq: assetToAdd1, dai: assetToAdd2 },
        },
        fund: { comptrollerProxy, fundOwner },
      } = await provider.snapshot(snapshot);
  ...
  describe('__removeZeroBalanceTrackedAssets', () => {
    it('only allows authorized users', async () => {
      const {
        ...
        deployment: {
          integrationManager,
          tokens: { risq: assetToRemove1, dai: assetToRemove2 },
        },
        fund: { comptrollerProxy, fundOwner },
      } = await provider.snapshot(snapshot);
