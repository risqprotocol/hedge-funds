...
describe('shares', () => {
  ...
  test('Shares contract is properly initialized', async () => {
    const backOfficeName = await mockSystem.backOffice.methods.name().call();
    await expect(shares.methods.name().call()).resolves.toEqual(backOfficeName);
    await expect(shares.methods.symbol().call()).resolves.toBe('RISQF');
