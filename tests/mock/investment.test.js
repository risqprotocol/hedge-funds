...
describe('investment', () => {
  ...
  beforeAll(async () => {
    ...
    defaultPbgu = toWei('0.01', 'ether');

    const price = toWei('1', 'ether');
    await mockSystem.priceSource.methods
      .update(
        [mockSystem.weth.options.address, mockSystem.risq.options.address],
        [price, price],
      )
      .send(defaultTxOpts);
  ...
  test('Invest fails in shut down fund', async () => {
    const errorMessage = 'BackOffice is shut down';
    const amount = toWei('1', 'ether');
    await mockSystem.backOffice.methods.setShutDownState(true).send(defaultTxOpts);
    ...
    await mockSystem.backOffice.methods.setShutDownState(false).send(defaultTxOpts);
    ...
    await mockSystem.participation.methods
      .requestInvestment(amount, amount, mockSystem.weth.options.address)
      .send({ ...defaultTxOpts, value: defaultPbgu });
