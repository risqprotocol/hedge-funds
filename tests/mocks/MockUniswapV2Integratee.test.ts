...
import { MockUniswapV2PriceSource } from '@risqprotocol/protocol';
...
async function snapshot(provider: EthereumTestnetProvider) {
  ...
  const token0 = deployment.tokens.risq;
...
describe('getAmountsOut', () => {
  it('correctly retrieves getAmountsOut from an integratee', async () => {
    ...
    const path = [knc.address, risq.address];
...
describe('removeLiquidity', () => {
  it('correctly removes liquidity and receives corresponding tokens', async () => {
    const {
      config: { deployer },
      deployment: {
        tokens: { risq: token0, knc: token1, weth: referenceToken },
        valueInterpreter,
        uniswapV2Integratee,
      },
      mocks: { mockPair },
