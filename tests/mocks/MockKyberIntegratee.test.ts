...
import { createNewFund, kyberTakeOrder, randomizedTestDeployment } from '@risqprotocol/testutils';
...
it('correctly retrieves expectedRate from an integratee', async () => {
  const {
    deployment: {
      ...
      tokens: { knc, risq },
      chainlinkAggregators: { knc: aggregatorKnc, risq: aggregatorRisq },
      kyberIntegratee,
    },
  ...
  const answerRisq = utils.parseEther('1');

  await aggregatorKnc.setLatestAnswer(answerKnc, BigNumber.from('1'));
  await aggregatorRisq.setLatestAnswer(answerRisq, BigNumber.from('1'));
  ...
  const { rate_ } = await kyberIntegratee.getExpectedRate.args(knc.address, risq.address, amount).call();
...
it('receives the expected amount of assets from a kyber swap integration', async () => {
  ...
  const { rate_, worstRate_ } = await kyberIntegratee.getExpectedRate
    .args(knc.address, risq.address, utils.parseEther('1'))
    .call();

  const preBalance = await risq.balanceOf(deployer.address);
  await kyberIntegratee.swapTokenToToken(knc.address, utils.parseEther('1'), risq.address, 1);
  const postBalance = await risq.balanceOf(deployer.address);
