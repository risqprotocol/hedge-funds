...
contract Registry is DSAuth {
    ...
    event RisqTokenChange(address indexed risqToken);
    ...
    function setRisqToken(address _risqToken) external auth {
        risqToken = _risqToken;
        emit RisqTokenChange(_risqToken);
    }
    ...
    function isFund(address _who) external view returns (bool) {
        if (fundsToVersions[_who] != address(0)) {
            return true; // directly from a backOffice
        } else {
            BackOffice backOffice = BackOffice(Runner(_who).backOffice());
            require(
                backOffice.isRunner(_who),
                "Call from either a runner or backOffice"
            );
            return fundsToVersions[address(backOffice)] != address(0);
        }
    }
