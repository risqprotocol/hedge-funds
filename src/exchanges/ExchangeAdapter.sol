...
contract ExchangeAdapter is DSMath {
    ...
    modifier notShutDown() {
        require(
            !backOfficeShutDown(),
            "BackOffice must not be shut down"
        );
        _;
    }
    ...
    function getBackOffice() internal view returns (BackOffice) {
        return BackOffice(getTrading().backOffice());
    }

    function getAccounting() internal view returns (Accounting) {
        return Accounting(getBackOffice().accounting());
    }

    function backOfficeShutDown() internal view returns (bool) {
        return getBackOffice().isShutDown();
    }

    function getManager() internal view returns (address) {
        return getBackOffice().manager();
    }
