...
/// @author Risq Protocol <dev@risq.capital>
...
contract EthfinexAdapter is DSMath, ExchangeAdapter {
        ...
        public
    {
        ...
        address nativeAsset = Accounting(backOffice.accounting()).NATIVE_ASSET();
     ...
     /// @notice Minor: Wrapped tokens directly sent to the fund are not accounted. To be called by Trading runner
        ...
        internal
    {
        ...
        if (_makerAsset == nativeAsset) {
            Vault vault = Vault(backOffice.vault());
    ...
    /// @dev Function to be called from Trading runner context (Delegate call)
    ...
    /// @dev Function to be called by Trading runner without change of context (Non delegate call)
