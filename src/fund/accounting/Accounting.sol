...
contract Accounting is PbguConsumer, Runner {
    ...
    constructor(address _backOffice, address _denominationAsset, address _nativeAsset)
        public
        Runner(_backOffice)
    {
...
contract AccountingFactory is Factory {
    ...
    function createInstance(address _backOffice, address _denominationAsset, address _nativeAsset) external returns (address) {
        address accounting = address(new Accounting(_backOffice, _denominationAsset, _nativeAsset));
        childExists[accounting] = true;
        emit NewInstance(_backOffice, accounting, _denominationAsset, _nativeAsset);
        return accounting;
    }
}
