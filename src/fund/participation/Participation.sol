...
contract Participation is TokenUser, PbguConsumer, Runner {
    ...
    constructor(address _backOffice, address[] memory _defaultAssets, address _registry)
        public
        Runner(_backOffice)
    {
    ...
    function cancelRequest() external payable pbguPayable(false) {
    ...
    function engine() public view override(PbguConsumer, Runner) returns (address) { return Runner.engine(); }
    function risqToken() public view override(PbguConsumer, Runner) returns (address) { return Runner.risqToken(); }
    function priceSource() public view override(PbguConsumer, Runner) returns (address) { return Runner.priceSource(); }
    function registry() public view override(PbguConsumer, Runner) returns (address) { return Runner.registry(); }
}
...
contract ParticipationFactory is Factory {
    ...
    function createInstance(address _backOffice, address[] calldata _defaultAssets, address _registry)
        external
        returns (address)
    {
        address participation = address(
            new Participation(_backOffice, _defaultAssets, _registry)
        );
        childExists[participation] = true;
        emit NewInstance(_backOffice, participation, _defaultAssets, _registry);
