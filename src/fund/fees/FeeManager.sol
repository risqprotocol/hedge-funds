...
contract FeeManager is DSMath, Runner {
    ...
    constructor(address _backOffice, address _denominationAsset, address[] memory _fees, uint[] memory _rates, uint[] memory _periods, address _registry) Runner(_backOffice) public {
    ...
    function _rewardFee(IFee fee) internal {
        ...
        fee.updateState();
        Shares(routes.shares).createFor(backOffice.manager(), rewardShares);
...
contract FeeManagerFactory is Factory {
    function createInstance(
        ...
        address feeManager = address(
            new FeeManager(_backOffice, _denominationAsset, _fees, _feeRates, _feePeriods, _registry)
        );
        childExists[feeManager] = true;
        emit NewInstance(_backOffice, feeManager);
        return feeManager;
    }
}
