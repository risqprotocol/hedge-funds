...
contract MaxConcentration is TradingSignatures, DSMath {
    ...
    function rule(bytes4 sig, address[5] calldata addresses, uint[3] calldata values, bytes32 identifier)
        ...
        returns (bool)
    {
        Accounting accounting = Accounting(BackOffice(Trading(msg.sender).backOffice()).accounting());
        ...
        if (sig == MAKE_ORDER) {
            IPriceSource priceSource = IPriceSource(BackOffice(Trading(msg.sender).backOffice()).priceSource());
